...
import { CategoryReminderManager } from './CategoryReminderManager';
import { CategoryWidgetManager } from './CategoryWidgetManager';
import { 
  ArrowLeft, 
  Calendar, 
  Edit3, 
...
interface CategoryDetailProps {
  category: Project;
  onBack: () => void;
  onEdit?: (category: Project) => void;
}

type SortOption = 'newest' | 'oldest' | 'field';

export function CategoryDetail({ category, onBack, onEdit }: CategoryDetailProps) {
  ...
  // Category icon is an emoji string

  // Get entries for this project
  useEffect(() => {
    const projectEntries = state.entries.filter(entry => 
      entry.projectId === category.id || entry.categoryId === category.id
    );
    setEntries(projectEntries);
  }, [state.entries, category.id]);
  ...
  const getCategoryStats = () => {
    const reminders = state.reminders.filter(reminder => (reminder.projectId || reminder.categoryId) === category.id);
    const widgets = state.widgets.filter(widget => (widget.projectId || widget.categoryId) === category.id);
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Are you sure you want to delete "{category.name}"? This will also delete all {stats.entries} entries, {stats.reminders} reminders, and {stats.widgets} widgets. This action cannot be undone.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
