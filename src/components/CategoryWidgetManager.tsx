...
import { Category, Widget, FieldType } from '../types';
...
interface CategoryWidgetManagerProps {
  category: Category;
  isOpen: boolean;
  onClose: () => void;
}
...
export function CategoryWidgetManager({ category, isOpen, onClose }: CategoryWidgetManagerProps) {
  ...
  // Filter widgets for this category only
  const categoryWidgets = state.widgets.filter(w => w.categoryId === category.id);
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-3">
            ...
            Home Screen Widgets for {category.name}
          </DialogTitle>
          <DialogDescription>
            Create quick entry widgets for your home screen to track {category.name} data
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <Badge variant="outline">{categoryWidgets.filter(w => w.isEnabled).length} active</Badge>
            </div>
            <Button
            ...
            <Card className="border-dashed border-muted-foreground/30">
              <CardContent className="text-center py-6">
                <p className="text-muted-foreground">
                  ...
                  Add fields like Number, Scale, Rating, or Tally to create widgets.
                </p>
              </CardContent>
            </Card>
          )}
          ...
          {/* Widget Preview */}
          {categoryWidgets.filter(w => w.isEnabled).length > 0 && (
                ...
                <div className="relative w-full max-w-xs mx-auto aspect-[9/16] bg-black rounded-3xl p-1">
                  <div className="w-full h-full bg-white dark:bg-gray-900 rounded-[22px] p-4 overflow-hidden">
                    ...
                    <div className="grid grid-cols-4 gap-2 relative">
                      {categoryWidgets.filter(w => w.isEnabled).map((widget) => (
                        <div
                          key={widget.id}
                          className={`
              ...
              <Card className="text-center py-8">
                <CardContent>
                  ...
                  <p className="text-muted-foreground mb-4">
                    Create your first home screen widget for quick {category.name} tracking
                  </p>
                </CardContent>
              </Card>
            ) : (
        ...
        <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Set up a home screen widget for {category.name}
              </DialogDescription>
            </DialogHeader>
            ...
            <Tabs defaultValue="basic" className="space-y-4">
              ...
              <TabsContent value="basic" className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input
                    id="widget-name"
                    placeholder={`e.g., Quick ${category.name} Log`}
