...
import { Widget, FieldType } from '../types';
...
export function WidgetManager() {
  ...
  const handleCreateWidget = () => {
    if (!newWidget.categoryId || !newWidget.fieldId || !newWidget.name) return;

    const widgetToAdd = newWidget as Omit<Widget, 'id'>;
  ...
  const getFieldType = (categoryId: string, fieldId: string): FieldType => {
    const category = state.projects.find(c => c.id === categoryId);
    const field = category?.fields.find(f => f.id === fieldId);
  ...
  const getAvailableFields = (categoryId: string) => {
    ...
    return category.fields.filter(field => 
      ['number', 'boolean', 'scale', 'rating', 'tally', 'choice'].includes(field.type)
    );
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground max-w-[60%] mr-4">
            Create quick entry widgets for your home screen
          </p>
        </div>
        <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Set up a home screen widget for quick data entry
              </DialogDescription>
            </DialogHeader>
            ...
            <Tabs defaultValue="basic" className="space-y-4">
              ...
              <TabsContent value="basic" className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input
                    id="widget-name"
                    placeholder="e.g., Quick Water Log"
                ...
                {newWidget.categoryId && (
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => {
                        const fieldType = getFieldType(newWidget.categoryId!, value);
                        setNewWidget({ 
                          ...newWidget, 
                          fieldId: value,
                          type: fieldType
                    ...
                    >
                      ...
                      <SelectContent>
                        {getAvailableFields(newWidget.categoryId).map((field) => (
                    ...
                    {getAvailableFields(newWidget.categoryId!).length === 0 && (
                      <p className="text-sm text-muted-foreground">
                        No suitable fields found for widgets in this category
                      </p>
                    )}
                  </div>
                )}
                  ...
                  <div className="flex gap-2 overflow-x-auto pb-2">
                    {colorOptions.map((color) => (
                      <button
                        key={color.value}
                        type="button"
                        className={`w-10 h-10 rounded-lg flex-shrink-0 ${color.class} ${
            ...
            <div className="relative w-full max-w-xs mx-auto aspect-[9/16] bg-black rounded-3xl p-1">
              <div className="w-full h-full bg-white dark:bg-gray-900 rounded-[22px] p-4 overflow-hidden">
                ...
                <div className="grid grid-cols-4 gap-2 relative">
                  {state.widgets.filter(w => w.isEnabled).map((widget) => (
                    <div
                      key={widget.id}
                      className={`
          ...
          <Card className="text-center py-8">
            <CardContent>
              ...
              <p className="text-muted-foreground mb-4">
                Create your first home screen widget for quick data entry
              </p>
            </CardContent>
          </Card>
        ) : (
                  ...
                  <CardHeader className="pb-3">
                    <div className="flex items-start justify-between">
                      <div className="space-y-1">
                        ...
                        <CardDescription>
                          {getCategoryName(widget.categoryId)} • {getFieldName(widget.categoryId, widget.fieldId)}
                        </CardDescription>
                      </div>
                      <div className="flex items-center gap-2">
                        <Button
